...
5. Name of the file that contains the measurements on heart rate and oxygen consumption.
...
- CartesianFrameSignal: This class is in charge of configuring the main container for the HR and the VO2 signals. It sets the principal characteristics of each of them (i.e. The minimum and maximum values a signal can achieve). If the dataset changes it is necessary to modify the signal characteristics accordingly to the new dataset (panel.setAxes(...) method).

- BigPanel: This class is in charge of containing the two panels for the HR and the VO2 signals. It is used to set the preferred size of the main panel each time a change occurs (new data is included).
